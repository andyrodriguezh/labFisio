...
function showRockportReference(result, age, gender) {
    ...
    // NUEVA IMPLEMENTACIÓN: Determinar categoría del resultado
    let categoryIndex = -1;
        ...
        if (ageGroupData[categoryIndex].rank && ageGroupData[categoryIndex].rank !== '-') {
            ...
            while (j >= 0) {
                if (ageGroupData[j].rank && ageGroupData[j].rank !== '-') {
                    ...
                    percentile = ageGroupData[j].percentile;
                    break;
                }
                j--;
            }
    ...
    // Generar etiquetas para la barra de rango
    const minVO2 = gender === 'M' ? 17 : 13;
    const maxVO2 = gender === 'M' ? 62 : 52;
    ...
    // Posicionar el marcador en la barra
